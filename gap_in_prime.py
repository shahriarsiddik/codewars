...
# exist otherwise nil or null or None or Nothing (depending on the language). In C++ return in such a case {0, 0}.
# Examples:
...
def gap(g, from_num, to_num):
    first_prime = 0
    for num in range(from_num,to_num+1):
        if num>1:
            for num_iter in range(2,int(math.sqrt(num))+1):
                if num%num_iter==0:
                    break
            else:
                if num-g==first_prime:
                    return [first_prime,num]
                else:
                    first_prime = num
print gap(8,300,400)
